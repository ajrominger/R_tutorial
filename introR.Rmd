...
Whatever the task to be completed in `R`, we do so using commands. Commands are given using
...
mark (`?`). For example `?rnorm` provides us with information about the use of the function, its
...
Suppose we wanted to store a sample of 10 random normal numbers, rather than just print it to the screen as we did in the previous example. To do so we must
tell `R` what name to store those values under, let's use the name `my.norm`.
```{r}
my.norm <- rnorm(n=10, mean=1, sd=2)
```
This is called "assignment" and we would say "assign 10 random normal values to an object named `my.norm`." It uses a special assignment operator `<-`; it is equivilant to use an equal sign `=`, but the special assignment operator is prefered in `R`.

By again typing `my.norm` (and running it) we print those 10
numbers; note they will be different from the previous example because those were two different samples form `R`'s normal random number generator
```{r}
my.norm
```
Now that these values are stored in the object `my.norm` we can use them for any ruther computation, for example, simple addition
```{r}
my.norm + 2
```
...
For the sake of really making this clear, let's recap: we made this object with the "assign" operator, i.e. the `<-`
...
it is good etiquette to reserve the assig operator `<-` for assigning values to objects and only use the equal operator `=` for specifying arguments in functions (see Table [basicOperators]).

           Command                                              
  ---------------- -------------------------------------------- -------------------
              `<-` Assignment of value to object memory         `x <- 5,`
  ...
  : Basic operators and arithmetic functions. Many functions can be used with much finesse, which we can't summarize here. Consult the help pages, see section [sec:help].<span data-label="basicOperators"></span>


## A note on objects and classes in R

`R` is an object-oriented programming language, meaing it operates on objects (values which we've assigned a name to in memory) and those objects additionally are automaticallly characterized by their class. Not knowing the details of what that means is not a major
limitation; however it is helpful to have a basic understanding. All
objects belong to a class and this class identity
...
(perhaps using some function `build.wall(method='human')`), but no function
can use a wall to build a human. `R` has a huge number classes and we'll learn some key ones as we go. Some examples are `numeric`, which is the class
of our `my.norm` object, and `data.frame`, which is the class of spread
sheet like objects in `R` and the default class for any data read into
...
numbers (class `numeric` or `integer`), characters (class `character`), or booleans (`TRUE`/`FALSE`
values; class `logical`). Vectors can be created using various functions, for example we
...
Vectors are commonly used in `R` functions,
such as in conducting a t-test (testing the means of two
*vectors*) or in plotting data points (plotting two
...
only the second element of the vector `x` (recall that the values of `x` are \texttt{`r x`}), we would simply type:
```{r}
x[2]
```
...
x[c(1, 3)]
```
We could equally well use `TRUE`'s and `FALSE`'s to indicate which
elements we desire (recall the vector `z` composed of the booleans \texttt{`r z`}):
```{r}
x[z]
```
...
in a vector, for example we can change the first element of `y` to a capital `A`.
```{r}
y
y[1] <- "A"
y
...
Numeric vectors can be added, subtracted, multiplied and divided[^1a]. `R`
...
in linear algebra[^1b]. Examples will help illuminate
```{r}
my.vector <- 1:4
ur.vector <- 5:8
my.vector + 1
...
to the section on more advanced statistics (section [sec:sim]). We
...
# Simulation and Permutation {#sec:sim}

# Writing Your Own Functions {#sec:funs}



[^1a]: Note: this is an example where classes determine the possible operations that can be done on a object; a vector of class `character` cannot be operated on by `+, - *`, etc.

[^1b]: See in section [sec:matrix] for linear algebra operations
